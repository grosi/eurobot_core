/*****************************************************************************
 * \brief		CMSIS Cortex-M4 Device Peripheral Access Layer System Source
 *				File. This file contains the system clock configuration for
 *				STM32F4xx devices, and is generated by the clock
 *				configuration tool stm32f4xx_Clock_Configuration_V1.1.0.xls
 *
 ...
 *							Multiplier and Divider factors, AHB/APBx
 *							prescalers and Flash settings), depending on the
 *							configuration made in the clock xls tool.
 *							This function is called at startup just after
 *							reset and before branch to main program. This
 *							call is made inside the "startup_stm32f4xx.s"
 *							file.
 *
 *			- SystemCoreClock variable:	Contains the core clock (HCLK), it
 *										can be used by the user application
 *										to setup the SysTick timer or
 *										configure other parameters.
 *
 *			- SystemCoreClockUpdate():	Updates the variable SystemCoreClock
 *										and must be called whenever the core
 *										clock is changed during program
 *										execution.
 *
 *	2.	After each device reset the HSI (16 MHz) is used as system clock
 *		source. Then SystemInit() function is called, in "startup_stm32f4xx.s"
 ...
 *		SystemInit() function will do nothing and HSI still used as system
 *		clock source. User can add some code to deal with this issue inside
 *		the SetSysClock() function.
 *
 ...
 *============================================================================
 *****************************************************************************
 * \attention &copy; COPYRIGHT 2013 STMicroelectronics
 *
 ...
 *				(HCLK), it can be used by the user application to setup the
 *				SysTick timer or configure other parameters.
 *
 * \note		Each time the core clock (HCLK) changes, this function must
 *				be called to update SystemCoreClock variable value. Otherwise,
 *				any configuration based on this variable will be incorrect.
 *
 * \note		- The system frequency computed by this function is not the
 ...
 *					 (default value 16 MHz) but the real value may vary
...
void SystemCoreClockUpdate(void) {
	...
	tmp = AHBPrescTable[((RCC ->CFGR & RCC_CFGR_HPRE )>> 4)]; /* Get HCLK
																 prescaler	*/
 ...
 *
 * \note		This function should be called only once the RCC clock
 *				configuration is reset to the default reset state (done in
 *				SystemInit() function).
 *
 * \return		None
 *****************************************************************************
...
static void SetSysClock(void) {
	...
	if (HSEStatus == (uint32_t) 0x01) {
		...
		RCC ->PLLCFGR = PLL_M | (PLL_N << 6)
		        ...
		        | (PLL_Q << 24);			/* Configure the main PLL	*/
		...
		FLASH ->ACR = FLASH_ACR_PRFTEN | FLASH_ACR_ICEN | FLASH_ACR_DCEN
		        | FLASH_ACR_LATENCY_5WS;	/* Configure Flash prefetch,
